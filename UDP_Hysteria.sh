...
show_usage_and_exit() {
	echo
	echo -e "\t$(tbold)$SCRIPT_NAME$(treset) - AlienWare Tunnel server install script"
	echo
	echo -e "Usage:"
	echo
	echo -e "$(tbold)Install AlienWare Tunnel$(treset)"
	...
	echo -e "\t-l, --local <file>\tInstall specified AlienWare Tunnel binary instead of download it."
	echo -e "\t--version <version>\tInstall specified version instead of the latest."
	echo
	echo -e "$(tbold)Remove AlienWare Tunnel$(treset)"
	echo -e "\t$0 --remove"
	echo
...
tpl_hysteria_server_service_base() {
  ...
  cat << EOF
[Unit]
Description=AlienWare Tunnel Service
					...
					if [[ -z "$_is_update_required" ]]; then
						...
						if [[ -n "$_is_frash_install" ]]; then
							...
							echo
								...
								echo
								echo -e "$(tbold) UDP has been successfully update to $VERSION.$(treset)"
								echo
								fi
}
...
perform_remove() {
	...
	echo
	echo -e "$(tbold)Congratulation! UDP has been successfully removed from your server.$(treset)"
	echo
...
start_services() {
	echo "Starting AlienWare Tunnel"
	apt update
